...
def dhcp_leases_show():
    ...
    for ovs_rec in dhcp_leases.idl.tables["dnsmasq_leases"].rows.itervalues():
        dhcp_lease_entry = {"time":"*", "mac_addr":"*", "ip_addr":"*",
                        "client_hostname":"*", "client_id":"*"}

        if ovs_rec.time and ovs_rec.time != None:
            dhcp_lease_entry["time"] = ovs_rec.time
        ...
        if ovs_rec.client_hostname and ovs_rec.client_hostname != None:
            dhcp_lease_entry["client_hostname"] = ovs_rec.client_hostname
        if ovs_rec.client_id and ovs_rec.client_id != None:
            dhcp_lease_entry["client_id"] = ovs_rec.client_id
...
def main():
    ...
    if num_args > 2:
        ...
        dhcp_lease_entry["time"] = os.environ["DNSMASQ_LEASE_EXPIRES"]
    if num_args > 3:
