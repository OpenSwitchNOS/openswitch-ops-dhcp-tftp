...
to all servers failed"
    ...
    cmdOut = dut01.cmd("ps aux | grep dnsmasq")
    assert '-a 1.1.1.1' in cmdOut, "Test to set source-ip address \
to all servers failed"
    return True
...
to all servers failed"
    ...
    cmdOut = dut01.cmd("ps aux | grep dnsmasq")
    assert '-i 2' in cmdOut, "Test to set source-ip address \
to all servers failed"
    return True
...
def unsetSourceIpToAllServersTest(dut01):
    ...
    cmdOut = dut01.cmd("ps -ef | grep dnsmasq")
    assert '-i 2' not in cmdOut, "Test to set source-ip address \
to all servers failed"
    return True
